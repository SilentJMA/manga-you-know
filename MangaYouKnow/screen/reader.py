...
class MangaReader:
    ...
    def create_content(self):
        self.chapters = self.page.data.get('manga_chapters', [])
        self.pages = self.page.data.get('chapter_images', [])
        ...
        self.images_b64.sort(key=lambda e: e[1])
        ...
        is_second_time = self.content is not None

        if not is_second_time:
            self.content = ft.Stack([
                ft.Row(self.images, alignment=ft.MainAxisAlignment.CENTER),
                ft.Row([
                ...
                if img.visible:
                    if len(self.images) > i + 1:
                        ...
                        if len(self.images) == i + 2:
                            self.db.set_manga(self.page.data['id'], 'id_last_readed',
                                               self.page.data['id_chapter'])
        ...
        def on_key(e: ft.KeyboardEvent):
            if e.key == keybinds.get('next-page'):
                next()
            if e.key == keybinds.get('previous-page'):
                previous()
            if e.key == keybinds.get('full-screen'):
            ...
            if e.key == keybinds.get('return-home'):
        ...
        self.content.data = {'resize': resize}
